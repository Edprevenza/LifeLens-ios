            ...
            GeometryReader { geometry in
                ZStack {
                    ...
                    Rectangle()
                        ...
                        .offset(x: -geometry.size.width/2 + animationProgress * geometry.size.width)
                }
            }
            .frame(height: 200)
            .background(
    ...
    var body: some View {
        GeometryReader { geometry in
            Path { path in
                ...
                // Vertical lines
                stride(from: 0, to: geometry.size.width, by: gridSize).forEach { x in
                    path.move(to: CGPoint(x: x, y: 0))
                    path.addLine(to: CGPoint(x: x, y: geometry.size.height))
                }
                
                // Horizontal lines
                stride(from: 0, to: geometry.size.height, by: gridSize).forEach { y in
