...
class CloudMLResponseHandler: ObservableObject {
    ...
    struct ActivePrediction: Identifiable {
        let id = UUID()
    ...
    private func processTimeSeries(_ timeSeries: TimeSeries<Float>, type: PredictionType) -> ActivePrediction? {
        ...
        return ActivePrediction(
            type: type,
            value: maxRisk,
            confidence: timeSeries.confidence[maxIndex],
            timeToEvent: timeToEvent,
            severity: getSeverity(for: maxRisk),
            actionRequired: maxRisk > Self.HIGH_RISK_THRESHOLD
        )
    }
    ...
    private func mergeWithEdgeResults(_ cloudPredictions: [ActivePrediction]) async -> [ActivePrediction] {
        ...
        for cloudPred in cloudPredictions {
            if let edgePred = await getEdgePrediction(for: cloudPred.type) {
                // Weighted average
                let mergedValue: Any
                let mergedConfidence: Float
                
                if let cloudValue = cloudPred.value as? Float,
                   let edgeValue = edgePred.value as? Float {
                    mergedValue = cloudValue * Self.CLOUD_WEIGHT + edgeValue * Self.EDGE_WEIGHT
                    mergedConfidence = cloudPred.confidence * Self.CLOUD_WEIGHT + 
                                     edgePred.confidence * Self.EDGE_WEIGHT
                } else {
                    mergedValue = cloudPred.value
                    mergedConfidence = cloudPred.confidence
                }
                ...
                let mergedPrediction = ActivePrediction(
                    ...
                    timeToEvent: cloudPred.timeToEvent,
                    severity: cloudPred.severity,
                    actionRequired: cloudPred.actionRequired
                )
    ...
    private func getEdgePrediction(for type: PredictionType) async -> ActivePrediction? {
        switch type {
        case .arrhythmia:
            ...
            return ActivePrediction(
                ...
                timeToEvent: 0,
                severity: result.confidence > 0.7 ? .high : .moderate,
                actionRequired: result.confidence > 0.8
            )
        ...
        case .hypoglycemia:
            ...
            let result = await edgeML.predictGlucose(
                history: trend,
                timestamps: trend.map { _ in Date() },
                currentTrend: 0
            )
            
            if result.predictedValue < 70 {
                return ActivePrediction(
                    type: type,
                    value: (70 - result.predictedValue) / 70,
                    confidence: result.confidence,
                    timeToEvent: TimeInterval(result.timeToEvent * 60),
                    severity: result.predictedValue < 54 ? .critical : .high,
                    actionRequired: true
                )
            }
            return nil
    ...
    private func shouldGenerateAlert(for prediction: ActivePrediction) -> Bool {
        let key = String(describing: prediction.type)
        ...
        return cooldownExpired || prediction.severity == .critical
    }
    ...
    private func createAlert(from prediction: ActivePrediction) -> HealthAlert {
        ...
        return HealthAlert(
            ...
            title: title,
            message: message,
            type: String(describing: prediction.type),
            severity: String(describing: prediction.severity),
            priority: getNotificationPriority(for: prediction.severity),
            timestamp: Date(),
            actionRequired: prediction.actionRequired,
            timeToEvent: prediction.timeToEvent,
            confidence: prediction.confidence
        )
    }
    
    private func getAlertTitle(for type: PredictionType) -> String {
        switch type {
    ...
    private func buildAlertMessage(for prediction: ActivePrediction) -> String {
        let risk = Int(((prediction.value as? Float) ?? 0) * 100)
        let timeStr = formatTimeToEvent(prediction.timeToEvent)
        
        switch prediction.type {
        case .hypoglycemia:
            return "Blood sugar may drop below 70 mg/dL \(timeStr). Risk: \(risk)%. Consider eating 15g of carbohydrates."
        case .arrhythmia:
            return "Irregular heart rhythm detected. Risk level: \(risk)%. Monitor symptoms and consider medical consultation."
        case .myocardialInfarction:
            return "Elevated heart attack risk (\(risk)%) detected \(timeStr). Seek immediate medical attention if experiencing chest pain."
        default:
            return "Health risk detected: \(prediction.type). Risk level: \(risk)%. Time to event: \(timeStr)"
        }
    }
    ...
    private func getNotificationPriority(for severity: Severity) -> NotificationPriority {
        switch severity {
    ...
    private func createMonitoringRecommendation() -> HealthRecommendation {
    ...
    private func triggerEmergencyProtocol(for prediction: ActivePrediction) async {
        ...
        await alertManager.sendEmergencyAlert(
            ...
            data: [
                "type": String(describing: prediction.type),
                "risk": String(describing: prediction.value),
...
class RecommendationEngine {
    func generateRecommendation(for prediction: ActivePrediction) -> HealthRecommendation? {
        switch prediction.type {
        case .hypoglycemia:
    ...
    private func generateHypoglycemiaRecommendation(_ prediction: ActivePrediction) -> HealthRecommendation {
        return HealthRecommendation(
            category: .emergency,
            title: "Prevent Low Blood Sugar",
            description: "Your glucose is predicted to drop. Take preventive action.",
            priority: prediction.severity == .critical ? .emergency : .high,
            actions: [
                RecommendedAction(
            ...
            expiresAt: Date().addingTimeInterval(prediction.timeToEvent)
        )
    }
    
    private func generateHyperglycemiaRecommendation(_ prediction: ActivePrediction) -> HealthRecommendation {
        return HealthRecommendation(
            category: .medication,
    ...
    private func generateArrhythmiaRecommendation(_ prediction: ActivePrediction) -> HealthRecommendation {
        return HealthRecommendation(
            category: .monitoring,
            title: "Heart Rhythm Monitoring",
            description: "Irregular heart rhythm detected. Monitor closely.",
            priority: prediction.severity == .critical ? .urgent : .high,
            actions: [
                RecommendedAction(
                ...
                RecommendedAction(
                    ...
                    urgency: prediction.severity == .critical ? .immediate : .withinDay,
    ...
    private func generateFallPreventionRecommendation(_ prediction: ActivePrediction) -> HealthRecommendation {
        return HealthRecommendation(
            category: .lifestyle,
